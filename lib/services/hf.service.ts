...
import { HFSyncLog } from '../lib/types';

export const HFService = {
  async syncMessages(): Promise<{ synced: number; lastId: string | null }> {
  ...
  async syncJobs(): Promise<{ synced: number }> {
    const adminClient = getAdminClient();
  ...
  async pushToDataset(split: string, rows: unknown[]): Promise<void> {
  ...
  async createDatasetRepo(): Promise<void> {
    const token = process.env.HF_TOKEN;
    const repo = process.env.HF_DATASET_REPO;
  ...
  async getLastSyncInfo(): Promise<HFSyncLog[]> {
    const adminClient = getAdminClient();
