...
import { AISummary, AIAgentTask, Message, Conversation } from '../lib/types';
...
export const AIService = {
  async summarizeConversation(conversationId: string, hoursBack: number = 24): Promise<AISummary> {
...
Use bullet points. Keep it brief but informative.`;
    ...
    return savedSummary as AISummary;
  },

  async summarizeAllChats(hoursBack: number = 24): Promise<AISummary[]> {
    const adminClient = getAdminClient();
...
Be concise and informative.`;
  ...
  async processJobAutonomously(jobId: string, adminUserId: string): Promise<any> {
    const adminClient = getAdminClient();
